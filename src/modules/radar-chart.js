 ...
 * Radar Chart Module
 * Simplified layout, stable hover tooltips, updated year selection logic
 */
...
>>>>>>> 6af34461d7899493105a629f0deb16ee85623f73
...
<<<<<<< HEAD
        ...
        const points = pointGroup
            .selectAll(null)
            .data(serie.values.map((value, idx) => ({ ...value, angleIndex: idx, color: serie.color, year: serie.year })))
=======
            .attr('d', radarLine)
            .attr('stroke', color)
...
<<<<<<< HEAD
        ...
        points.on('pointerenter', function () {
            d3.select(this).select('.radar-marker').classed('is-active', true);
        })
            .on('pointerleave', function () {
=======
...
<<<<<<< HEAD
        group
            .on('pointerenter', () => {
                ...
                points.classed('is-hovered', true);
=======
...
>>>>>>> 6af34461d7899493105a629f0deb16ee85623f73
            ...
            .on('pointerleave', () => {
                ...
                points.classed('is-hovered', false);
            })
            .on('pointermove', (event) => {
                const centroid = d3.pointer(event, wrapper.node());
                tooltip.style('left', `${centroid[0]}px`).style('top', `${centroid[1] - 24}px`);
...
export function setupYearSelection(data, onUpdateCallback, country = 'Portugal') {
    ...
    const allYears = data.years || [];
    const displayYears = [...allYears].sort((a, b) => b - a);
    const defaultYears = determineDefaultYears(displayYears);
    ...
    if (!selectedYears.length) {
        d3.select('#viz-inflation-categories').html(
            renderEmptyState({
                ...
                message: 'Escolha um ano para analisar a inflação por categoria.',
                meta: 'O radar oferece mais clareza com apenas um ano ativo.',
                icon: '🗓️',
            }),
        );
        return;
...
function determineDefaultYears(years) {
    ...
    const sorted = [...years].sort((a, b) => a - b);
    return sorted.slice(-MAX_DEFAULT_VISIBLE_YEARS);
}
